...
画面（TUI）に選択肢が出ます。矢印キーで移動し、スペースキーでオン・オフを切り替え、エンターで確定します。この教材の構成にするには、次のオプションにチェックを入れてください。

- **必ず**: `unstable-hal`（ADCやPWMなどに必要）、`embassy`（この教材の全コードの前提）、`probe-rs`（書き込み・ログ）、`defmt`（ログの出し方）
- **無線を使うプロジェクトなら**: `alloc`、`wifi`、`ble-trouble` も追加

`log` は `probe-rs` と同時に選べない点にだけ注意してください（probe-rsならログは `defmt`）。オプションの意味や、コマンド一発で作るヘッドレス生成については、付録の[esp-generateでプロジェクトを作る](/embassy-esp32-c6/appendix/esp-generate/)にまとめてあります。確定したら、生成されたフォルダへ移動します。
...
[features]
default = ["probe-rs"]
probe-rs = ["dep:rtt-target"]   # 既定: ログをRTT経由でprobe-rsが読む
espflash = ["dep:esp-println"]  # 代替: ログをUSBシリアルへ流しespflashが読む

[dependencies]
esp-hal = { version = "~1.1.0", features = ["esp32c6", "unstable", "defmt"] }
esp-rtos = { version = "0.3.0", features = ["esp32c6", "embassy", "defmt"] }
esp-bootloader-esp-idf = { version = "0.5.0", features = ["esp32c6", "defmt"] }
esp-backtrace = { version = "0.19.0", features = ["esp32c6", "panic-handler", "defmt"] }
defmt = "1.0.1"
rtt-target = { version = "0.6.2", features = ["defmt"], optional = true }
esp-println = { version = "0.17.0", features = ["esp32c6", "defmt-espflash"], optional = true }
...
- **defmt**: ログを書くための軽量なフォーマット層。コードでは`defmt::info!`のように使います
- **rtt-target / esp-println**: defmtで書いたログの「出口」。既定のprobe-rsではrtt-target、代替のespflashではesp-printlnを使います（`optional = true`で片方だけ有効化）
- **esp-backtrace**: エラー（panic）が起きたときの情報表示
- **embassy-executor / embassy-time**: taskの実行と時間待ち

`features = ["esp32c6", ...]`は「このクレートのESP32-C6向け機能を有効にする」という指定です。同じクレートが複数のチップに対応しているため、必ずチップ名を指定します。`[features]`の`default = ["probe-rs"]`は、何も指定しなければprobe-rs用のログ出口を使うという設定です。espflashに切り替える方法は[次のページ](/embassy-esp32-c6/part01/09-flash-monitor/)で扱います。
...
プログラムの各部分をメモリのどこへ置くかを決める「リンカスクリプト」を指定しています。OSのないマイコンでは、この配置指定が必須です（メモリの話は第5部で扱います）。`-Tdefmt.x`はdefmtログのための追加スクリプトで、**`-Tlinkall.x`より前**に書く決まりです。
...
- `runner = "probe-rs run ..."`: `cargo run`と打ったとき、ビルド後に**自動でprobe-rsが書き込みとログ表示までやる**設定です。espflashで書き込みたいときは、この行をespflash用に差し替えます（[次のページ](/embassy-esp32-c6/part01/09-flash-monitor/)で説明）
- `DEFMT_LOG = "info"`: defmtログの表示レベル（infoレベル以上を表示）
...
2. ビルドされたプログラムをprobe-rsでボードへ書き込み、続けてdefmtのログを表示します。
...
- `cargo run`一発でビルド→書き込み→ログ表示まで動くように設定されている（既定はprobe-rs、代替はespflash）

## 次のページ

プロジェクトができたので、いよいよボードへ書き込みます。probe-rsの動きと、defmtのログを見る方法を学びます。
